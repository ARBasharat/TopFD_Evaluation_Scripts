...
def determine_artifacts(features):
  ...
  for init_idx in range(0, len(features)):
    ...
    if len(shortlisted_features) > 0:
      ...
      for f_idx in shortlisted_features:
        ...
        if check_in_list(mass, low_harmonic_masses, error_tol):
          ...
          # if feature.Abundance < f.Abundance:
            # print(init_idx, feature.MonoMass, (feature.MinCharge, feature.MaxCharge), feature.Abundance, feature.Score)
            # print("--->",init_idx, f_idx, "low_harmonics", f.MonoMass, (f.MinCharge, f.MaxCharge), f.Abundance, f.Score)
          ...
          # if feature.Abundance < f.Abundance:
            # print(init_idx, feature.MonoMass, (feature.MinCharge, feature.MaxCharge), feature.Abundance, feature.Score)
            # print("--->",init_idx, f_idx, "high_harmonics", f.MonoMass, (f.MinCharge, f.MaxCharge), f.Abundance, f.Score)
            feature_cluseter.append(f)
          ...
          # if feature.Abundance < f.Abundance:
            # print(init_idx, feature.MonoMass, (feature.MinCharge, feature.MaxCharge), feature.Abundance, feature.Score)
            # print("--->", init_idx, f_idx, "isotopologues", f.MonoMass, (f.MinCharge, f.MaxCharge), f.Abundance, f.Score)
            feature_cluseter.append(f)
      ...
      #   ids = [i.FeatureID for i in feature_cluseter]
      #   if any(x in abundance for x in ids):
      #     print(init_idx, ids, [i.Score for i in feature_cluseter], [i.Abundance for i in feature_cluseter])
          # break
...
output_dir = r"C:\Users\Abdul\Documents\topfd_cpp_results\Sep_19_v3\Artifact_removal"
...
###################
output_dir = create_directory(output_dir, "TopFD_all_f_new_score")
print("\nLoad Features")  
...
for file in feature_files:
  ...
  multicharge_features = [i for i in multicharge_features if i.Score >=0.1]
  multicharge_features_replicate[file_idx] = multicharge_features

# features = multicharge_features_replicate[1]
# Number of Valid features: 11830 and Artifacts: 136, 79, 22
# Percentage of Valid features: 98.04% and Artifacts: 1.13% 0.65% 0.18%
## 11809 258

# Number of Valid features: 11763 and Artifacts: 170, 90, 44
# Percentage of Valid features: 97.48% and Artifacts: 1.41% 0.75% 0.36%
...
for rep_idx in range(1, len(multicharge_features_replicate) + 1):
  ...
  x, y, z = determine_artifacts(topfd_feature_info)
...
# # # topfd_features_status = []
# # # for rep_idx in range(9, len(multicharge_features_replicate) + 1):
# # #   print("Processing Replicate", rep_idx)
# # #   topfd_feature_info = multicharge_features_replicate[rep_idx]
# # #   determine_artifacts(topfd_feature_info)
# # #   topfd_features_status.append(topfd_feature_info)
...
# # # # rep_idx = 1
# # # # topfd_feature_info = deepcopy(topfd_features_replicate[rep_idx])
# # # # topfd_feature_info.featureList.sort(key=lambda x: (x is None, x.Abundance), reverse=False)
# # # # low_harmonics, high_harmonics, isotopologues, charge_off_by_on = determine_artifacts(topfd_feature_info)
