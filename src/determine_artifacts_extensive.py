...
# Number of Valid features: 7228 and Artifacts: 231
# Percentage of Valid features: 96.9% and Artifacts: 3.1%
def determine_artifacts(features):
  ...
  for init_idx in range(0, len(features)):
    # if init_idx%2000 == 0: print("Processing", init_idx, "of", len(features))
    ...
    if len(shortlisted_features) > 0:
      ...
      status = [i.t_status for i in feature_cluseter]
  ...
  artifact_ids = [f.FeatureID for f in features if f.status != "Valid"]
...
output_dir = create_directory(os.path.dirname(topfd_feature_dir), "Artifact_removal_classified")
